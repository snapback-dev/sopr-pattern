		...
		for (const [toolName, toolDef] of Object.entries(TOOL_MAP)) {
			for (const [modeName, modeDef] of Object.entries(toolDef.modes)) {
				it(`${toolName}.${modeName} has a non-empty handler name`, () => {
					expect(modeDef.handler, `Mode "${toolName}.${modeName}" has no handler name defined`).toBeTruthy();
				...
				it(`${toolName}.${modeName} has at least one service dependency`, () => {
					// help tool is pure documentation — no service dependencies by design
					if (toolName === "help") {
						expect(
							modeDef.services.length,
							`Pure help mode "${toolName}.${modeName}" should have zero service dependencies.`,
						).toBe(0);
						return;
					}
					expect(
	...
	describe("Tool Name Conventions", () => {
		for (const [toolName, toolDef] of Object.entries(TOOL_MAP)) {
			it(`"${toolName}" key matches its name property`, () => {
				expect(toolName, `Tool map key "${toolName}" does not match name property "${toolDef.name}"`).toBe(
					toolDef.name,
				);
			});

			it(`"${toolName}" uses lowercase naming`, () => {
				expect(toolName, `Tool name "${toolName}" should be lowercase`).toBe(toolName.toLowerCase());
			});
		}
	});
});
