...
describe("createSnapHandlers", () => {
	...
	describe("context", () => {
		it("returns both state and learnings", async () => {
			...
			const result = await handlers.context({ mode: "context", intent: "debug", files: ["src/x.ts"] }, ctx);
...
describe("createCheckHandlers", () => {
	...
	describe("patterns", () => {
		it("delegates to validationService.checkPatterns with code and filePath", async () => {
			...
			const result = await handlers.patterns({ mode: "patterns", code: "let x = 1;", file: "src/a.ts" }, ctx);
...
describe("createGraphHandlers", () => {
	describe("deps", () => {
		it("delegates to graphService.computeDependencyGraph with params", async () => {
			...
			const result = await handlers.deps({ mode: "deps", entryPoint: "src/index.ts", depth: 5 }, ctx);
	...
	describe("files", () => {
		it("delegates to graphService.computeFileGraph with params", async () => {
			...
			const result = await handlers.files({ mode: "files", entryPoint: "src/main.ts", depth: 2 }, ctx);
...
describe("createCacheHandlers", () => {
	...
	describe("patterns", () => {
		it("delegates to cacheService.getPatterns with key filter", async () => {
			...
			const result = await handlers.patterns({ mode: "patterns", key: "no-console", value: "refresh" }, ctx);
...
describe("createLearnHandlers", () => {
	describe("load", () => {
		it("delegates to learningService.loadTiered with intent and filePaths", async () => {
			...
			const result = await handlers.load({ mode: "load", intent: "debug", filePaths: ["src/a.ts"] }, ctx);
	...
	describe("save", () => {
		...
		it("defaults to pattern type when type is omitted", async () => {
			...
			expect(learningService.save).toHaveBeenCalledWith(expect.objectContaining({ type: "pattern" }));
		});
		...
		it("defaults trigger and action to empty string when omitted", async () => {
			...
			expect(learningService.save).toHaveBeenCalledWith(expect.objectContaining({ trigger: "", action: "" }));
		});
		...
		it("maps all short type codes correctly", async () => {
			...
			for (const [shortCode, expectedType] of mappings) {
				await handlers.save({ mode: "save", trigger: "t", action: "a", type: shortCode as "pat" }, ctx);

				expect(learningService.save).toHaveBeenLastCalledWith(expect.objectContaining({ type: expectedType }));
			}
		});
	});
	...
	describe("search", () => {
		it("delegates to learningService.search with query and type", async () => {
			...
			const result = await handlers.search({ mode: "search", query: "auth pattern", type: "pat" }, ctx);
