...
import type { GitCommandRunner, GitHubFetcher, SentryFetcher } from "../../src/services/integration-service.js";
...
function createService(
	opts: { gitRunner?: GitCommandRunner; sentryFetcher?: SentryFetcher; githubFetcher?: GitHubFetcher } = {},
) {
	...
	const service = new IntegrationServiceImpl({}, logger, gitRunner, opts.sentryFetcher, opts.githubFetcher);
...
describe("IntegrationServiceImpl", () => {
	...
	describe("enrichContext", () => {
		...
		it("risk score does not exceed 1.0", async () => {
			const sentryFetcher = createSentryFetcher({
				errorRate: 0.5,
				recentErrors: [{ id: "1", title: "fatal", count: 10, lastSeen: Date.now(), level: "fatal" }],
			});
			const gitRunner = createGitRunner({
				"rev-parse --abbrev-ref HEAD": "main\n",
			...
			const coreFiles = ["auth.ts", "security.ts", "payment.ts", "config.ts", "database.ts", "migration.ts"];
