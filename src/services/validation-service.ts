...
// ---------------------------------------------------------------------------
// Timeout constants
// Used by safeRunCommand and the circular-deps graph service call.
// ---------------------------------------------------------------------------

/** Default subprocess timeout for tsc / eslint / vitest runs (ms). */
export const COMMAND_TIMEOUT_MS = 20_000;

/** Timeout for circular-dependency graph analysis via madge (ms). */
export const GRAPH_ANALYSIS_TIMEOUT_MS = 10_000;

/** Standard Unix exit code for a timed-out process. */
...
export class ValidationServiceImpl implements IValidationService {
	...
	async checkPatterns(input: PatternValidationInput): Promise<ServiceResult<PatternValidationResult>> {
		try {
	...
	private async recordEvolutionSnapshot(workspacePath: string, snapshot: EvolutionSnapshot): Promise<void> {
		try {
