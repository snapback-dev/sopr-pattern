...
const WIRE_PREFIX = "\u{1F9E2}"; // Cap emoji used as wire-format sentinel
const SEPARATOR = "|";
...
export function decode(
  ...
  if (parts.length < 2 || parts[0] !== WIRE_PREFIX) {
    throw new WireFormatError(
      `Invalid wire format: expected prefix "${WIRE_PREFIX}", got "${parts[0] ?? ""}"`,
    );
  }
  ...
  for (let i = 2; i < parts.length; i++) {
    ...
    if (colonIdx === -1) {
      throw new WireFormatError(
        `Malformed field at position ${i}: "${field}" (expected "key:value")`,
      );
    }
...
export function encodeCheck(payload: {
  passed: boolean;
  errorCount: number;
  warningCount: number;
}): string {
...
export function encodePulse(payload: {
  status: string;
  serviceCount: number;
  uptime: number;
}): string {
