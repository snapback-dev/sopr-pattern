...
export class LearningServiceImpl implements ILearningService {
	...
	async save(input: SaveLearningInput): Promise<ServiceResult<Learning>> {
		try {
			...
			const duplicate = existing.find((l) => l.trigger === input.trigger && l.action === input.action);
	...
	async search(input: SearchLearningsInput): Promise<ServiceResult<SearchLearningsResult>> {
		try {
			...
			matches = matches.filter(
				(l) => l.trigger.toLowerCase().includes(queryLower) || l.action.toLowerCase().includes(queryLower),
			);
