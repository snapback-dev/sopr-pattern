 ...
 *
 * The `structuredContent` field enables typed machine-readable output
 * alongside the human-readable `content` blocks. When a tool has an
 * `outputSchema`, the registry populates this automatically from the
 * handler's return value.
 */
export interface CallToolResult {
	content: TextContent[];
	/** Typed structured output matching the tool's outputSchema. */
