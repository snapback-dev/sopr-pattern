 ...
 *
 * Ensures @snapback-oss/sopr-mcp contains NO proprietary references.
 * Prevents IP leakage by construction: these tests fail if any SnapBack-specific
 * references leak into the OSS codebase.
 *
...
// From tests/integration/ → snapback-mcp/
 ...
 * OSS must not import from any @snapback/* (non-OSS) packages.
 */
const FORBIDDEN_IMPORT_PATTERNS = [
	/from\s+["']@snapback\//, // @snapback/* (proprietary scope)
	/import\s+.*["']@snapback\//, // import ... from "@snapback/..."
	/require\s*\(\s*["']@snapback\//, // require("@snapback/...")
];
...
describe("OSS IP Leakage Guard — @snapback-oss/sopr-mcp", () => {
	...
	describe("should NOT import from @snapback/* (proprietary scope)", () => {
		for (const filePath of sourceFiles) {
			const rel = relative(PKG_ROOT, filePath);
			it(`${rel} has no @snapback/* imports`, () => {
		...
		for (const filePath of sourceFiles) {
			...
			it(`${rel} has no proprietary patterns`, () => {
				...
				for (const { pattern, reason } of FORBIDDEN_PATTERNS) {
					expect(content.includes(pattern.toLowerCase()), `Found "${pattern}" in ${rel} — ${reason}`).toBe(
						false,
					);
				}
			});
	...
	describe("package.json validation", () => {
		...
		it("should be scoped under @snapback-oss", () => {
			expect(pkgJson.name).toMatch(/^@snapback-oss\//);
		});
		...
		it("should NOT have a telemetry/ directory with PostHog references", () => {
			...
			if (existsSync(telemetryDir)) {
				...
				for (const file of files) {
					...
					expect(content.includes("posthog"), `Found PostHog reference in ${relative(PKG_ROOT, file)}`).toBe(
						false,
					);
				}
			}
