...
const BUILT_IN_RULES: readonly SecurityRule[] = [
	...
	// Insecure deserialization
	{
		...
		cwe: "CWE-502",
		recommendation: "Validate and sanitize input before parsing. Use a schema validation library like zod.",
	},

	// Path traversal
	{
		id: "SEC007",
		pattern: /(?:readFile|readFileSync|createReadStream|access|stat)\s*\(\s*(?:req|params|body|query|input|user)/gi,
		severity: "high",
	...
	{
		id: "SEC008",
		pattern: /(?:rejectUnauthorized\s*:\s*false|NODE_TLS_REJECT_UNAUTHORIZED\s*=\s*["']0["']|verify\s*:\s*false)/g,
		severity: "high",
		message: "TLS/SSL certificate verification disabled",
		cwe: "CWE-295",
		recommendation: "Never disable certificate verification in production. Use proper certificate management.",
	},
	...
	// Weak cryptography
	{
		...
		cwe: "CWE-328",
		recommendation: "Use SHA-256 or stronger hash algorithms. For passwords, use bcrypt, scrypt, or Argon2.",
	},
	...
	// Exposed debug/stack trace
	{
		...
		cwe: "CWE-209",
		recommendation: "Log full stack traces server-side but return only sanitized error messages to clients.",
	},
	...
	// Hardcoded IP addresses (non-localhost)
	{
		...
		recommendation: "Use configuration or environment variables for network addresses instead of hardcoding.",
	},
	...
	// innerHTML / dangerouslySetInnerHTML
	{
		...
		cwe: "CWE-79",
		recommendation: "Sanitize HTML content before injection. Use DOMPurify or framework-provided sanitization.",
	},
];
...
export class SecurityServiceImpl implements ISecurityService {
	...
	private scanContent(content: string, filePath: string, rules: readonly SecurityRule[]): SecurityFinding[] {
		const findings: SecurityFinding[] = [];
