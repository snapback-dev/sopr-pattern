...
describe("CacheServiceImpl", () => {
	...
	describe("getErrors", () => {
		...
		it("respects limit parameter", async () => {
			...
			for (let i = 0; i < 10; i++) {
				service.errorCache.set(`error:/ws:e${i}`, makeCachedError({ id: `e${i}`, occurrences: 10 - i }));
			}
	...
	describe("getPatterns", () => {
		...
		it("filters patterns by patternFilter (case-insensitive)", async () => {
			...
			service.patternCache.set("pattern:/ws:p1", makeCachedPattern({ id: "p1", patternName: "no-console" }));
			service.patternCache.set("pattern:/ws:p2", makeCachedPattern({ id: "p2", patternName: "no-eval" }));
			service.patternCache.set("pattern:/ws:p3", makeCachedPattern({ id: "p3", patternName: "NO-CONSOLE-log" }));
