...
export type DirectoryLister = (dirPath: string) => Promise<Array<{ name: string; isDirectory: boolean; path: string }>>;
...
function parseExports(content: string): string[] {
	...
	const namedExportRegex = /export\s+(?:default\s+)?(?:const|let|var|function|class|type|interface|enum)\s+(\w+)/g;
	let match: RegExpExecArray | null;
...
export class GraphServiceImpl implements IGraphService {
	...
	async computeDependencyGraph(input: DependencyGraphInput): Promise<ServiceResult<DependencyGraphResult>> {
		try {
			...
			const entryPoints = input.entryPoints && input.entryPoints.length > 0 ? [...input.entryPoints] : allFiles;

			// Build the graph
			for (const filePath of entryPoints) {
				await this.buildGraph(filePath, input.workspacePath, nodes, edges, visited, 0, depth, allFiles);
			}
		...
		try {
			...
			if (graphResult.ok) {
				...
				if (fileGraphResult.ok) {
					...
					modularity =
						clusterCount > 0 && totalEdges > 0 ? Math.min(1, clusterCount / Math.sqrt(totalEdges)) : 0;
				}
			}
	...
	private resolveImportPath(fromFile: string, importSource: string, allFiles: string[]): string | null {
