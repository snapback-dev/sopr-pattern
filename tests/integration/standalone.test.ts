 ...
 *
 * Proves that @snapback-oss/sopr-mcp works entirely standalone:
 ...
 * without any SnapBack proprietary infrastructure.
 *
...
describe("Standalone Execution — @snapback-oss/sopr-mcp", () => {
	describe("Static Analysis: No Proprietary Imports", () => {
		...
		it("src/ has NO @snapback/* imports (only @snapback-oss/* allowed)", () => {
	...
	describe("Default Implementations Are Provided", () => {
		...
		it("exports NoOpTelemetry as default ITelemetry implementation", () => {
			...
			telemetry.track({ event: "test", tool: "test", mode: "test", tier: "free", durationMs: 0, success: true });
		});
		...
		it("all modes have valid service references", () => {
			...
			for (const [toolName, tool] of Object.entries(TOOL_MAP)) {
				for (const [modeName, mode] of Object.entries(tool.modes)) {
					...
					if (toolName === "help") {
						expect(mode.services, `${toolName}.${modeName} should have empty services`).toHaveLength(0);
						continue;
					}
	...
	describe("Tool Registry Standalone Operation", () => {
		...
		it("can execute tools with mock context (no external services)", async () => {
			...
			const result = await registry.execute("echo", { mode: "echo", message: "hello standalone" }, context);
	...
	describe("Phase 4a: Read-Only Tool Definitions (pulse + help)", () => {
		...
		const noOpValidationService = {
			...
			checkBuild: async () => ({ ok: true as const, data: { success: true, diagnostics: [], duration: 0 } }),
			checkCoverage: async () => ({
				ok: true as const,
				data: { totalPercent: 100, filesCovered: 0, filesTotal: 0, belowThreshold: [], passed: true },
			}),
			computeHealthScore: async () => ({
				ok: true as const,
				data: { overall: 95, dimensions: {}, trend: "stable" as const },
			}),
			analyzeEvolution: async () => ({ ok: true as const, data: { snapshots: [], trend: "stable" as const } }),
		};
		...
		const noOpGraphService = {
			...
			detectCircularDeps: async () => ({ ok: true as const, data: { cycles: [], affectedFiles: [] } }),
			detectOrphans: async () => ({
				ok: true as const,
				data: { orphanedFiles: [], deadExports: [], totalFiles: 0 },
			}),
			computeFileGraph: async () => ({ ok: true as const, data: { nodes: [], edges: [], clusters: [] } }),
			computeHealth: async () => ({
				ok: true as const,
		...
		it("createHelpToolDef creates a valid help ToolDefinition", () => {
			...
			expect(Object.keys(helpDef.modes)).toEqual(
				expect.arrayContaining(["tools", "status", "wire", "modes", "thresholds", "decision", "all"]),
			);
