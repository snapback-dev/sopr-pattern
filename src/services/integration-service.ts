...
export class IntegrationServiceImpl implements IIntegrationService {
			...
			const recentCommits: GitCommit[] = logOutput
				...
				.map((line) => {
					...
					return {
						...
						timestamp: Number.parseInt(parts[3] ?? "0", 10) * 1000, // Convert to ms
					};
				});
	...
	async checkConfig(input: IntegrationConfigInput): Promise<ServiceResult<IntegrationConfigResult>> {
		try {
	...
	private computeRiskScore(
		...
		if (git && git.status === "dirty") {
			risk += 0.1;
			if (git.uncommittedFiles.length > 20) {
				risk += 0.1;
			}
		}
