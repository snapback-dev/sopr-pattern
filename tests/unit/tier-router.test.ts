...
vi.mock("../../src/router/daemon-client.js", () => ({
  checkDaemonHealth: vi.fn(),
  DaemonClient: vi.fn(),
}));

import { checkDaemonHealth } from "../../src/router/daemon-client.js";

const mockCheckHealth = checkDaemonHealth as ReturnType<typeof vi.fn>;
...
describe("TierRouter", () => {
  ...
  describe("delegate()", () => {
    ...
    it("throws when no daemon client configured", async () => {
      ...
      await expect(router.delegate("snap", {})).rejects.toThrow("Daemon client not configured");
    });
  });
  ...
  describe("createUpgradePrompt()", () => {
    it("generates upgrade prompt with feature list", () => {
      ...
      expect(prompt.message).toContain("SnapBack Pro");
