...
describe("ToolRegistry", () => {
	...
	describe("execute() — input validation", () => {
		...
		it("throws InvalidInputError when required field has wrong type", async () => {
			...
			await expect(registry.execute("test-tool", { mode: "run", count: "not-a-number" }, ctx)).rejects.toThrow(
				InvalidInputError,
			);
		});
	...
	describe("execute() — handler errors", () => {
		it("wraps handler errors in HandlerExecutionError", async () => {
			...
			await expect(registry.execute("test-tool", { mode: "start" }, ctx)).rejects.toThrow(HandlerExecutionError);
		});
	...
	describe("execute() — cancellation", () => {
		it("throws RequestCancelledError when signal is already aborted", async () => {
			...
			await expect(registry.execute("test-tool", { mode: "start" }, ctx)).rejects.toThrow(RequestCancelledError);
		});
