...
import { checkDaemonHealth, DaemonClient } from "../../src/router/remote-client.js";
...
describe("checkDaemonHealth", () => {
	...
	it("returns available=true when daemon responds OK", async () => {
		(fetch as ReturnType<typeof vi.fn>).mockResolvedValue(mockResponse({ version: "1.2.0", uptime: 3600 }));

		const health = await checkDaemonHealth("http://127.0.0.1:4201", 1000);
	...
	it("returns available=false on timeout", async () => {
		...
		const health = await checkDaemonHealth("http://127.0.0.1:4201", 1);
		expect(health.available).toBe(false);
	});
...
describe("DaemonClient", () => {
	...
	it("sends correct HTTP request for tool delegation", async () => {
		...
		expect(url).toBe("http://127.0.0.1:4201/tools/snap");
	...
	it("throws on non-OK daemon response", async () => {
		...
		await expect(client.callTool("snap", {})).rejects.toThrow("Delegate returned 500 for snap");
	});
	...
	it("encodes tool name in URL", async () => {
		...
		expect(url).toBe("http://127.0.0.1:4201/tools/risk-analysis");
	});
