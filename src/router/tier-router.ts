...
import { checkDaemonHealth, type DaemonClient } from "./daemon-client.js";
...
export interface TierRouterConfig {
  ...
  /** Port number for daemon detection (default: 4200). */
...
export function classifyTool(toolName: string): ToolTier {
  if (PRO_ONLY_TOOLS.has(toolName)) return "pro-only";
  if (UPGRADEABLE_TOOLS.has(toolName)) return "upgradeable";
  return "free";
}
...
export class TierRouter {
  ...
  constructor(config?: Partial<TierRouterConfig>, daemonClient?: DaemonClient | null) {
   ...
   *
   * @throws {Error} if no daemon client is configured.
   */
  async delegate(toolName: string, input: unknown): Promise<unknown> {
    if (!this.daemonClient) {
      throw new Error("Daemon client not configured — cannot delegate");
    }
  ...
  createUpgradePrompt(toolName: string): {
    ...
    return {
      upgradeRequired: true,
      tool: toolName,
      message: `${toolName} requires SnapBack Pro. Install the daemon for enhanced capabilities.`,
