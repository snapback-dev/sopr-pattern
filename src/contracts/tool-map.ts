...
/**
 * Core service names provided by the SOPR framework.
 *
...
/**
 * Extensible service name type.
 *
 * Core services are typed as literal strings. Consumers can extend this
 * with additional service names via the `(string & {})` escape hatch,
 * which allows arbitrary strings while preserving autocomplete for core
 * service names.
 */
 ...
 *
 * 8 tools, 34 modes, backed by 7 injectable services.
 *
...
export const TOOL_MAP = {
	...
	graph: {
		name: "graph",
		description: "Dependency analysis. Modes: deps (module dependency graph), files (file relationship map).",
		annotations: {
	...
	help: {
		...
		modes: {
			tools: {
				description: "List available SOPR tools with modes and descriptions.",
				handler: "handleHelpTools",
				services: [],
				execution: "single",
			},
			status: {
				description: "Current session and protocol status.",
				handler: "handleHelpStatus",
				services: [],
				execution: "single",
			},
			wire: {
				description: "Wire format reference and field decode guide.",
				handler: "handleHelpWire",
				services: [],
				execution: "single",
			},
			modes: {
				description: "Mode reference for all tools.",
				handler: "handleHelpModes",
				services: [],
				execution: "single",
			},
			thresholds: {
				description: "Auto-promotion and risk threshold reference.",
				handler: "handleHelpThresholds",
				services: [],
				execution: "single",
			},
			decision: {
				description: "Tool selection decision tree guide.",
				handler: "handleHelpDecision",
				services: [],
				execution: "single",
			},
			all: {
				description: "Comprehensive help covering all topics.",
				handler: "handleHelpAll",
				services: [],
				execution: "single",
			},
		},
...
export type ModeDefinitionFor<T extends ToolName, M extends ModeName<T>> = (typeof TOOL_MAP)[T]["modes"][M];
...
export const TOTAL_MODE_COUNT = Object.values(TOOL_MAP).reduce((sum, tool) => sum + Object.keys(tool.modes).length, 0);
